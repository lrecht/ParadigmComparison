...
type Point(x, y) as this =  
    ...
    member __.PointsEquals (p: Point) = 
        p.X = x && p.Y = y
...
type Cluster(centroid: Point) as this = 
    let mutable sumX = 0.0
    let mutable sumY = 0.0
    let mutable pointCount = 0
    member val Centroid = centroid with get,set
...
type KMeans(initialPoints, clusters: Cluster[]) = 
    ...
    member this.Compute () =
        ...
        while not converged do
            ...
            for point:Point in initialPoints do
