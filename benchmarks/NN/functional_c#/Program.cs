...
{
    using Neuron = ImmutableDictionary<string, ImmutableArray<double>>;
    using Layer = ImmutableArray<ImmutableDictionary<string, ImmutableArray<double>>>;

    //TODO: Faster dictionaries with enums?
    class Program
    {
        ...
        private static ImmutableArray<Layer> backwardsPropagateError(ImmutableArray<Layer> network, ImmutableArray<double> expected)
        {
            ...
            var errors = outt.Zip(expected, (neuron, exp) => exp - neuron["output"][0]).ToImmutableArray();
            
            Func<double, double> derivative = (outty) => outty * (1.0 - outty);
            Func<double, Neuron, Neuron> getError = (error, neuron) => neuron.SetItem("delta", ImmutableArray<double>.Empty.Add(error * derivative(neuron["output"][0])));
            ...
            Func<Layer, int, double> neuronError = (layer, i) => layer.Select(neuron => neuron["weights"][i] * neuron["delta"][0]).Sum();
        ...
        private static (ImmutableArray<Layer>, ImmutableArray<double>) forwardPropagate(ImmutableArray<double> row, ImmutableArray<Layer> network)
        {
            ...
            Func<ImmutableArray<double>, Neuron, Neuron> propagateNeuron = (inputs, neuron) => neuron.SetItem("output", ImmutableArray<double>.Empty.Add(transfer(activate(neuron["weights"], inputs, neuron["bias"][0]))));
            Func<Layer, ImmutableArray<double>> getOutput = (layer) => layer.Select(neuron => neuron["output"][0]).ToImmutableArray();
        ...
        private static ImmutableArray<Layer> initialiseNetwork(int nInput, int nHidden, int nOutput){
            Layer initMapsWIthWeights(int maps, int weights){
                return Enumerable.Range(0, maps)
                    .Select(_ => Neuron.Empty)
                    .Select(neuron => neuron
                        .Add("weights", Enumerable.Range(0, weights).Select(_ => rand.NextDouble()).ToImmutableArray())
                        .Add("bias", ImmutableArray<double>.Empty.Add(rand.NextDouble())))
                    .ToImmutableArray();
            }
        ...
        private static ImmutableArray<Layer> updateWeights(ImmutableArray<Layer> network, ImmutableArray<double> dataRow, double learningRate){
            var inputs = network.Select((l, i) => {
                if(i != 0)
                    return network[i - 1].Select(neuron => neuron["output"][0]).ToImmutableArray();
                else
            ...
            return network.Select((l, i) => {
                    ...
                    return l.Select(neuron => {
                        var updatedWeights = range.Select(j => neuron["weights"][j] + (learningRate * neuron["delta"][0] * layerInputs[j])).ToImmutableArray();
                        var updatedBias = neuron["bias"][0] + learningRate * neuron["delta"][0];
                        return neuron.SetItem("weights", updatedWeights).SetItem("bias", ImmutableArray<double>.Empty.Add(updatedBias));
                        }).ToImmutableArray();
                }).ToImmutableArray();
        }
