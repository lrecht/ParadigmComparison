...
{
    public class OOP : IBenchmark
    {
        string[] input;
        public void Preprocess() => input = File.ReadAllLines($"benchmarks/graph.csv");

        public int Run()
        {
            int i = 0;
            Edge[] edges = input.Select(v => Edge.FromCsv(v, i++)).ToArray();
    ...
    public class UnionFind
    {
        ...
        public UnionFind(int totalNodes)
        {
            ...
            // No verticies belong to a group
            for (int v = 0; v < vertexGroups.Length; v++)
                vertexGroups[v] = -1;
        }
