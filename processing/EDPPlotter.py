...
class EDPPrinter():
    def __init__(self, weight: int,outputfile):
        self.weight = weight
        self.outputfile = outputfile

    def computeEDP(self,results):
        edp = {}
        for k in results.observations:
            ...
            k = Key(benchmark=k.benchmark,paradigm=k.paradigm,language=k.language,metric=MetricType.PACKAGE)
            cpu = results.observations[k]
            k = Key(benchmark=k.benchmark,paradigm=k.paradigm,language=k.language,metric=MetricType.DRAM)
    ...
    def createOutput(self,edp,results):
        lines = []
